...
class PinExtender(commands.Cog):
    ...
    async def on_guild_channel_pins_update(self, channel, last_pin):
        ...
        if channel.id in self.extended_pins and len(await channel.pins()) == 50:
            ...
            # Modified this line to check if the message content is empty or a valid URL and use the :paperclip: emoji accordingly
            message_description = ":paperclip:" if (last_pinned_message.content == "" or (urllib.parse.urlsplit(last_pinned_message.content).scheme and urllib.parse.urlsplit(last_pinned_message.content).netloc)) and (last_pinned_message.attachments or last_pinned_message.embeds) else (last_pinned_message.content[:20] + "..." if len(last_pinned_message.content) > 20 else last_pinned_message.content)
