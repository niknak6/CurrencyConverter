...
class TikTokCog(commands.Cog):
    ...
    def __init__(self, bot):
        ...
        self.tiktok_pattern = re.compile(r"(?i)(.*?)(https?://)?((\*)\.)?tiktok\.com/(ZM\w+)?(.+)(.*)") # Modified line
    ...
    async def on_message(self, message):
        ...
        new_url = (tiktok_url.group(1) or "") + (tiktok_url.group(2) or "") + (tiktok_url.group(3) or "") + "vxtiktok.com/" + (tiktok_url.group(5) or "") + (tiktok_url.group(6) or "") # Modified line
        ...
        # Remove the memo text from the message content by replacing it with an empty string and removing any leading or trailing whitespace
        message_content = message.content.replace(memo_text, "").strip()
