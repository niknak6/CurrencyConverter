    ...
    for row in data:
        ...
        if any(char.isdigit() for char in date):
            ...
            if date_pattern.match(date):
                ...
                # New line: if start + timedelta(weeks=1) <= date_obj < start + timedelta(weeks=upcoming_weeks + 1):
                if start + timedelta(weeks=1) <= date_obj < start + timedelta(weeks=upcoming_weeks + 1):
                    bs = "\\"
