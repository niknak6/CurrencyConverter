...
class TreacheryPokemon(commands.Cog):
    ...
    async def spawn(self, ctx):
        ...
        if ctx.channel == spawn_channel:
            ...
            if self.last_spawn is None or (now - self.last_spawn).total_seconds() >= spawn_cooldown * 60 or await self.bot.is_owner(ctx.author):
                ...
                original_pokemon_id = pokemon_id  # Store the original ID
                ...
                async with aiohttp.ClientSession() as session:
                    ...
                    async with session.get(pokemon_url) as response:
                        if response.status == 200:
                            ...
                            self.original_pokemon_id = original_pokemon_id  # Store the original ID
                            self.current_level = await self.get_average_spawn_level(pokemon_id)

                            async with session.get(self.current_sprite) as sprite_response:
                                if sprite_response.status == 200:
                                    image_data = BytesIO(await sprite_response.read())
    ...
    async def get_average_spawn_level(self, pokemon_id):
        def get_evolution_form(evolution_data, pokemon_name):
            def traverse_chain(chain, form):
                if chain['species']['name'] == pokemon_name:
                    return form
                for evolution in chain['evolves_to']:
                    result = traverse_chain(evolution, form + 1)
                    if result:
                        return result
                return None

            return traverse_chain(evolution_data['chain'], 1)

        def has_further_evolutions(evolution_data, pokemon_name):
            ...
            return traverse_chain(evolution_data['chain'])

        async def get_encounter_levels(session, pid):
        ...
        async with aiohttp.ClientSession() as session:
            levels = await get_encounter_levels(session, pokemon_id)
            
            if not levels:
                species_url = f"https://pokeapi.co/api/v2/pokemon-species/{self.original_pokemon_id}"
                async with session.get(species_url) as response:
                    if response.status == 200:
                        species_data = await response.json()
                        levels = await get_encounter_levels(session, self.original_pokemon_id)
            ...
            async with session.get(species_url) as response:
                if response.status == 200:
                    data = await response.json()
                    if data.get('is_legendary', False) or data.get('is_mythical', False):
                        return self.apply_random_variation(random.randint(40, 70))

                    evolution_response = await session.get(data['evolution_chain']['url'])
                    if evolution_response.status == 200:
                        evolution_data = await evolution_response.json()
                        evolution_level = self.get_evolution_level(evolution_data, self.current_pokemon)
                        if evolution_level:
                            return self.apply_random_variation(evolution_level)

                        evolution_form = get_evolution_form(evolution_data, self.current_pokemon)
                        if evolution_form >= 3 or not has_further_evolutions(evolution_data, self.current_pokemon):
                            return self.apply_random_variation(20)
    ...
    async def silent_evolution_check(self, message, poketag, pokemon_name, level):
        ...
        if evolution_chain:
            ...
            if eligible_evolutions:
                ...
                if notify_enabled:
                    evolution_message = f"{pokemon_name.capitalize()} is ready to evolve! Use `!evolve {poketag.upper()}` to transform it now. To turn off these alerts, use: `!evolvenotify`"
    ...
    async def update_experience_and_level(self, message):
        ...
        MAX_LEVEL = 100
        REQUIRED_EXP = lambda level: round(0.02 * level ** 2 + 0.2 * level + 1)

        async def process_pokemon(poketag, level, experience):
            if level >= MAX_LEVEL:
                return level, experience, None

            new_exp = experience + 1
            if new_exp >= REQUIRED_EXP(level):
                new_level = level + 1
                pokemon_name = self.get_pokemon_name(message.author.id, poketag)
                await self.silent_evolution_check(message, poketag, pokemon_name, new_level)
                return new_level, 0, (pokemon_name, new_level) if new_level in LEVEL_MILESTONES else None
        ...
        for poketag in party + [row[0] for row in self.cur.execute(f'''
            SELECT poketag FROM pokedex 
            WHERE member_id = ? AND poketag NOT IN ({','.join(['?']*len(party))})
            ORDER BY RANDOM() LIMIT 1
        ''', (message.author.id, *party))]:
            level, experience = self.cur.execute('SELECT level, experience FROM pokedex WHERE member_id = ? AND poketag = ?', (message.author.id, poketag)).fetchone()
            new_level, new_exp, level_up_info = await process_pokemon(poketag, level, experience)
            self.cur.execute('UPDATE pokedex SET level = ?, experience = ? WHERE member_id = ? AND poketag = ?', (new_level, new_exp, message.author.id, poketag))
            if level_up_info:
                leveled_up.append(level_up_info)
    ...
    async def levelup(self, ctx, *, option: Optional[str] = None):
        ...
        if not await self.bot.is_owner(ctx.author):
            return await ctx.send("This command has been decommissioned and will be removed in a future update.")
